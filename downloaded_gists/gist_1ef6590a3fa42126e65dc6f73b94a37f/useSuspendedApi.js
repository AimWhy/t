...
function getCachedPromise(wrappedFetch, args = defaultArgs) {
  ...
  if (!promiseCache.get(wrappedFetch).get(argsKey)) {
    promiseCache.get(wrappedFetch).set(argsKey, wrappedFetch.fetch(...args));
  }
...
export function useMutation(wrappedFetch) {
  ...
  const mutate = useCallback(
    async (...args) => {
      setMutationState((preState) => ({ ...preState, loading: true }));
      let currentState;
      try {
        const data = await wrappedFetch.fetch(...args);
        currentState = { loading: false, error: false, data };
      } catch (error) {
        currentState = { loading: false, error, data: void 0 };
      }
